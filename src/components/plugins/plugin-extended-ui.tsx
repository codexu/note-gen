...
export function PluginExtendedUi({ block, scope, render, compact = false }: {
  ...
  if (block.type === 'toolbar' && settingsLayout) return <Item variant="outline" role="toolbar" aria-label={block.label}>
    <ItemContent className="min-w-0"><ItemTitle>{block.label}</ItemTitle></ItemContent>
    <ItemActions className="ml-auto flex-wrap">{block.actions.map(action => <PluginAction key={action.id} command={action.command} argument={action.argument} confirmation={action.confirmation} label={action.label} size={action.iconOnly ? 'icon-sm' : 'sm'} disabled={action.disabled} variant={action.variant ?? 'outline'}>{action.icon ? <PluginIcon name={action.icon} data-icon="inline-start" /> : null}{action.iconOnly ? null : action.label}</PluginAction>)}</ItemActions>
  </Item>
  if (block.type === 'toolbar') return <div role="toolbar" aria-label={block.label} className={cn("flex items-center gap-1", !compact && "flex-wrap")}>{block.actions.map(action => <PluginAction key={action.id} command={action.command} argument={action.argument} confirmation={action.confirmation} label={action.label} size={action.iconOnly ? 'icon-sm' : 'sm'} disabled={action.disabled} variant={action.variant ?? 'ghost'}>{action.icon ? <PluginIcon name={action.icon} data-icon="inline-start" /> : null}{action.iconOnly ? null : action.label}</PluginAction>)}</div>
  ...
  if (block.type === 'section') {
    if (!block.collapsible && settingsLayout) return <SettingSection title={block.title}>{render(block.blocks)}</SettingSection>
