...
export async function uploadImageByGithub(file: File) {
  ...
  try {
    ...
    const url = `https://api.github.com/repos/${username}/${RepoNames.image}/contents/${filename}`;
    const response = await fetch(url, requestOptions);
    ...
    if (response.status >= 200 && response.status < 300) {
      ...
      if (jsdelivr) {
        await fetch(`https://purge.jsdelivr.net/gh/${username}/${RepoNames.image}@main/${data.content.name}`)
        url = `https://cdn.jsdelivr.net/gh/${username}/${RepoNames.image}@main/${data.content.name}`
      }
      return url
    }
