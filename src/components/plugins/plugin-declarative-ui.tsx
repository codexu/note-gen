...
export function PluginDeclarativeUi({ document, scope, nested = false }: { document?: PluginUiDocument; scope: string; nested?: boolean }) {
  ...
  return <div className={cn('flex min-w-0 flex-col gap-4 break-words', nested ? 'contents' : document.blocks.length === 1 && (document.blocks[0].type === 'navigation-list' || document.blocks[0].type === 'item-list') ? 'p-1' : 'p-3')}>{document.blocks.map((block, index) => {
    if (block.type === 'layout' || block.type === 'section' || block.type === 'tabs' || block.type === 'toolbar' || block.type === 'item-list' || block.type === 'markdown' || block.type === 'badge' || block.type === 'empty' || block.type === 'loading') return <PluginExtendedUi key={'id' in block ? `${block.type}:${block.id}` : index} block={block} scope={scope} render={blocks => <PluginDeclarativeUi scope={scope} document={{ blocks }} nested />} />
    if (block.type === 'navigation-list') return <PluginNavigationList key={`navigation:${block.id}`} block={block} scope={scope} />
    if (block.type === 'separator') return <Separator key={index} />
    ...
    if (block.type === 'progress') return <div key={index} className="flex flex-col gap-2"><p className="text-sm">{block.label}</p><Progress value={block.value} aria-label={block.label} aria-valuenow={block.value} /></div>
    ...
    if (block.type === 'text') return <p key={index} className={cn('whitespace-pre-wrap text-sm', block.tone === 'muted' && 'text-muted-foreground', block.tone === 'warning' && 'text-destructive')}>{block.text}</p>
    ...
    return <div key={index} className="flex flex-wrap gap-2">{block.actions.map((action) => <PluginAction key={action.id} disabled={action.disabled} variant={action.variant} command={action.command} argument={action.argument}>{action.label}</PluginAction>)}</div>
  })}</div>
}
